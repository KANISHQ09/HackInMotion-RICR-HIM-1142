    ...
    <article className="flex flex-1 flex-col rounded-lg border border-zinc-200/80 bg-white p-6 shadow-[0_18px_45px_-28px_rgba(24,24,27,0.35)]">
      ...
      <ul className="grid gap-3">
        {visibleMilestones.length === 0 ? (
          <li className="rounded-lg border border-dashed border-zinc-300 bg-[#f7f3f2] p-4 text-center text-sm font-medium text-zinc-500">
            Completed goals will appear here.
          </li>
        ) : null}

        {visibleMilestones.map(([name, date]) => (
